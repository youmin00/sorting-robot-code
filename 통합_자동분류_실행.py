...
Usage:
  ...
  python "통합_자동분류_실행.py" --serial <DEVICE_SERIAL> --width 640 --height 480 --fps 30
...
def scene_positions_stable(
    ...
    """Compare an unordered set of object positions without relying on tracking IDs."""
    ...
    for reference_x, reference_y in reference:
        if not unmatched:
            return False
        distances = [
            float(np.hypot(now_x - reference_x, now_y - reference_y))
            for now_x, now_y in unmatched
        ]
        nearest_index = int(np.argmin(np.asarray(distances, dtype=np.float64)))
...
def send_dual_arm_plans(
    ...
    if right_plan is not None:
        right_contact = right_plan["contact"]
        sides.append(
            f"오른팔 {right_plan['size']}mm"
            f"(접촉 J1={right_contact['j1']:.1f}, J2={right_contact['j2']:.1f}, "
            f"J3={right_contact['j3']:.1f}, J4={right_contact['j4']:.1f})"
        )
    log_status(
        ("교차 순차 계획 전송: " if sequential else "양팔 동시 계획 전송: ")
...
def request_emergency_stop(arm_ser, conveyor_ser=None) -> None:
    log_warning("긴급 정지: 컨베이어와 양팔을 즉시 멈추고 공압을 끕니다.")
    if conveyor_ser is not None:
        conveyor_ser.write(b"X\n")
        conveyor_ser.flush()
...
def hold_system_on_arm_fault(conveyor_ser, context: str) -> None:
    """로봇팔에는 추가 명령을 보내지 않고 벨트만 정지해 현재 자세·공압을 유지한다."""
    if conveyor_ser is not None:
        try:
            conveyor_ser.write(b"X\n")
            conveyor_ser.flush()
        except Exception as exc:
            log_warning(f"통신 오류 후 컨베이어 정지 실패: {exc}")
    log_warning(
        f"{context}: 컨베이어를 정지했습니다. 로봇팔 자세와 공압은 유지합니다. "
        "상태를 확인한 뒤 X 또는 Z를 누르세요."
    )
...
class D435Camera:
    ...
    def _prefetch_scene_while_arms_place(self) -> None:
        ...
        for obj in objects:
            if (
                ...
                continue
            try:
                if object_in_left_workspace(obj):
                    build_arm_pick_plan(obj, "left")
                elif object_in_right_workspace(obj):
    ...
    def preview(self, save_dir: str = "captures") -> None:
        ...
        arm_ser = None
        try:
            ser = serial.Serial(STM32_PORT, STM32_BAUD, timeout=0)
            time.sleep(2.0)
            ser.reset_input_buffer()
            ser.write(b"S\n" if AUTO_START_CONVEYOR else b"X\n")
            ser.flush()
            log_status(f"컨베이어 STM32와 연결되었습니다. ({STM32_PORT})")
            if AUTO_START_CONVEYOR:
                log_status("컨베이어 자동 시작 명령 S를 보냈습니다.", PHASE_BELT_MOVING)
            else:
                log_status(
                    "컨베이어는 정지 상태입니다. 미리보기 창에서 s를 누르면 자동 작업을 시작합니다.",
                    PHASE_SYSTEM,
                )
        except Exception as exc:
            log_warning(f"컨베이어 STM32 연결 실패: {exc}")
            log_warning("컨베이어 없이 카메라와 로봇팔만 실행합니다.")
        try:
        ...
        while True:
            ...
            try:
                ...
                if ser is not None and ser.in_waiting > 0:
                    ...
                    if msg == "CHECK":
                        ...
                        # 벨트가 멈춘 뒤 좌표가 연속 프레임에서 안정되면 즉시 진행한다.
                        objects = self._wait_for_stable_scene(
                            CAMERA_SETTLE_SEC,
                        ...
                        initial_eligible_objects = [
                            obj for obj in initial_valid_objects
                            if object_in_left_workspace(obj) or object_in_right_workspace(obj)
                        ]

                        # 작업영역 안에 도달 가능한 객체가 있을 때만 벨트를 정지 상태로 유지한다.
                        if initial_eligible_objects:
                            objects = initial_eligible_objects
                        ...
                        # 처음부터 작업영역 안 객체가 없으면 바로 E 전송
                        else:
                            self.preserve_sort_slots = False
                            self.reserved_sort_slot_ids.clear()
                            if objects:
                                log_warning(
                                    f"카메라에 물체 {len(objects)}개가 잠깐 보였지만 "
                                    "작업영역 밖이므로 벨트를 계속 이동합니다."
                                )
                            else:
            ...
            if key == ord("x"):
                ...
                if arm_ser is not None:
                    wait_arm_reply(
                        arm_ser, ("EMERGENCY_DONE",), 60.0,
                        conveyor_ser=ser, allow_emergency_key=False
                    )
            ...
            if key == ord("z"):
                ...
                if ser is not None:
                    try:
                        ...
                        log_warning(f"컨베이어 정지/버퍼 초기화 실패: {exc}")

                # 로봇팔 동작을 취소하고 안전 기본 자세 복귀 요청
                if arm_ser is not None:
                    try:
                        ...
                        log_warning(f"로봇팔 안전 복귀 요청 실패: {exc}")
                        continue
                ...
                # 프레임 오류까지 복구할 수 있도록 RealSense 파이프라인 재시작
                try:
                    self.stop()
                    self.pipeline = rs.pipeline()
                    ...
                    log_warning(f"카메라 재시작 실패: {exc}")
                    continue

                log_status(
                    "Z 리셋이 완료되었습니다. 컨베이어는 정지 상태이며 s를 누르면 다시 시작합니다.",
                    PHASE_SYSTEM,
                )
                continue
            if key == ord("s"):
                if ser is None:
                    log_warning("컨베이어 COM 포트가 연결되지 않았습니다.")
                    continue
                try:
                    self.preserve_sort_slots = False
                    self.reserved_sort_slot_ids.clear()
                    ...
                    ser.write(b"S\n")
                    ser.flush()
                    log_status("S 입력: 컨베이어 자동 작업을 시작합니다.", PHASE_BELT_MOVING)
                except Exception as exc:
                    log_warning(f"컨베이어 시작 실패: {exc}")
                continue
            if key == ord("p"):
                if arm_ser is None:
                    log_warning("로봇팔 COM 포트가 연결되지 않았습니다.")
                    continue
                write_arm_line(arm_ser, "r\n")
                log_status(
                    "P 입력: 양쪽 로봇팔을 로딩 자세(90/120/56/0)로 이동합니다.",
                    PHASE_SYSTEM,
                )
                continue
